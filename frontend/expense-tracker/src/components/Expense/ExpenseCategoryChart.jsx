...
const ExpenseCategoryChart = ({ transactions, selectedCategory, onCategorySelect, onClearCategory }) => {
    ...
    const visibleCategoryData = showAllCategories ? categoryData : categoryData.slice(0, 3);
        ...
        <div className="card">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div className="min-w-0 flex-1">
                    <h5 className="text-lg">Expense by Category</h5>
                    <p className="mt-0.5 text-xs text-gray-400">
                        Pick a category to compare its total against the selected expense period.
                    </p>
                </div>
                <div className="w-full max-w-full md:w-auto md:max-w-[18rem]">
                    <div className="w-full rounded-full border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-600 shadow-sm dark:border-slate-700 dark:bg-slate-900/60 dark:text-slate-300 md:text-xs">
                        Total Expense: ${addThousandSeparator(totalAmount)}
                    </div>
                </div>
            </div>
            ...
            {categoryData.length > 0 ? (
                    ...
                    <div className="flex items-center justify-between gap-4 pt-1">
                        <div>
                            {categoryData.length > 3 && (
                                <button
                                    type="button"
                                    onClick={() => setShowAllCategories((currentValue) => !currentValue)}
                ...
                <div className="mt-8 rounded-2xl border border-dashed border-gray-200 bg-gray-50 px-6 py-12 text-center text-sm text-gray-500 dark:border-slate-700 dark:bg-slate-900/40 dark:text-slate-400">
                    Add a few expense entries to see the category breakdown.
                </div>
            )}
        </div>
    );
